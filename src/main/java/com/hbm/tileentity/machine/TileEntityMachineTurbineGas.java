...
public class TileEntityMachineTurbineGas extends TileEntityMachineBase implements IFluidStandardTransceiver, IEnergyProviderMK2, IControlReceiver, IGUIProvider, SimpleComponent, CompatHandler.OCComponent, IFluidCopiable, IRORValueProvider, ITickable, IConnectionAnchors {
	...
	public void update() {
		...
		if(!world.isRemote) {
			...
			for(int i = 0; i < 2; i++) { //fuel and lube
				...
				this.trySubscribe(tanks[i].getTankType(), world, pos.getX() + dir.offsetX * 2 + rot.offsetX, pos.getY(), pos.getZ() + dir.offsetZ * 2 + rot.offsetZ, dir);
			}
			//water
			this.trySubscribe(tanks[2].getTankType(), world, pos.getX() - dir.offsetX * 2 + rot.offsetX * -4, pos.getY(), pos.getZ() - dir.offsetZ * 2 + rot.offsetZ * -4, dir.getOpposite());
			this.trySubscribe(tanks[2].getTankType(), world, pos.getX() + dir.offsetX * 2 + rot.offsetX * -4, pos.getY(), pos.getZ() + dir.offsetZ * 2 + rot.offsetZ * -4, dir);
			//steam
			this.sendFluid(tanks[3], world, pos.getX() + dir.offsetZ * 6, pos.getY() + 1, pos.getZ() - dir.offsetX * 6, rot.getOpposite());
	...
	private void makePower(double consMax, int throttle) {
		...
		if(world.getTotalWorldTime() % 10 == 0) //lube consumption
	...
	public boolean hasPermission(EntityPlayer player) {
		return Vec3.createVectorHelper(pos.getX() - player.posX, pos.getY() - player.posY, pos.getZ() - player.posZ).length() < 25;
	}
	...
	public AxisAlignedBB getRenderBoundingBox() {
		...
		if(bb == null) {
			bb = new AxisAlignedBB(pos.getX() - 5, pos.getY(), pos.getZ() - 5, pos.getX() + 6, pos.getY() + 3, pos.getZ() + 6);
		}
		return bb;
	}
	...
	public FluidTankNTM[] getReceivingTanks() {
		return new FluidTankNTM[] { tanks[0], tanks[1], tanks[2] };
	}

	@Override
	public FluidTankNTM[] getSendingTanks() {
		return new FluidTankNTM[] { tanks[3] };
	}
