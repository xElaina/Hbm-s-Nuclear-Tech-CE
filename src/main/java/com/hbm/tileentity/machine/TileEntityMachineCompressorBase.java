...
public abstract class TileEntityMachineCompressorBase extends TileEntityMachineBase implements IGUIProvider, IControlReceiver, IEnergyReceiverMK2, IFluidStandardTransceiverMK2, IUpgradeInfoProvider, IFluidCopiable, ITickable, IConnectionAnchors {
    ...
    public void update() {
        ...
        if(!world.isRemote) {
            ...
            for(DirPos pos : getConPos()) {
                this.tryProvide(tanks[1], world, pos.getPos(), pos.getDir());
            }
    ...
    public void provideInfo(UpgradeType type, int level, List<String> info, boolean extendedInfo) {
        ...
        if(type == UpgradeType.SPEED) {
            info.add(TextFormatting.GREEN + "Generic compression: "+ I18nUtil.resolveKey(KEY_DELAY, "-" + (level == 3 ? 90 : level == 2 ? 80 : level == 1 ? 40 : 0) + "%"));
            info.add(TextFormatting.GREEN + "Recipe: "+ I18nUtil.resolveKey(KEY_DELAY, "-" + (100 - 100 / (level + 1)) + "%"));
        }
        if(type == UpgradeType.POWER) {
            info.add(TextFormatting.GREEN + I18nUtil.resolveKey(KEY_CONSUMPTION, "-" + (100 - 100 / (level + 1)) + "%"));
        }
