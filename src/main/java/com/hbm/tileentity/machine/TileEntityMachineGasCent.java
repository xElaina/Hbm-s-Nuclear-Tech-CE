...
public class TileEntityMachineGasCent extends TileEntityMachineBase implements ITickable, IEnergyReceiverMK2, IFluidStandardReceiver, IGUIProvider, IFFtoNTMF, IConnectionAnchors {
    ...
    public TileEntityMachineGasCent() {
        ...
        inventory = new ItemStackHandler(7) {
            ...
            public void setStackInSlot(int slot, ItemStack stack) {
                ...
                if (Library.isMachineUpgrade(stack) && slot == 6) SoundUtil.playUpgradePlugSound(world, pos);
            }
        };
    ...
    private boolean canEnrich() {
        if (power > 0 && this.inputTank.getFill() >= inputTank.getTankType().getFluidConsumed() && this.outputTank.getFill() + this.inputTank.getTankType().getFluidProduced() <= outputTank.getMaxFill()) {
            ...
            if (this.inputTank.getTankType().getIfHighSpeed())
                if (!(!inventory.getStackInSlot(6).isEmpty() && inventory.getStackInSlot(6).getItem() == ModItems.upgrade_gc_speed))
                    return false;
    ...
    public static class PseudoFluidTank {
        ...
        public void setTankType(GasCentrifugeRecipes.PseudoFluidType type) {
            ...
            if (type == null) this.type = GasCentrifugeRecipes.PseudoFluidType.NONE;
            else this.type = type;
