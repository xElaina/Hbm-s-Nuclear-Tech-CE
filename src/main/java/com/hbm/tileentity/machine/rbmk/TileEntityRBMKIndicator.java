...
@AutoRegister
public class TileEntityRBMKIndicator extends TileEntityLoadedBase implements ITickable, IGUIProvider, IControlReceiver, SimpleComponent, CompatHandler.OCComponent {
    ...
    /** OR of {@link IndicatorUnit#active} across all units; lets the TESR early-out when the panel is empty. */
    ...
    private void recomputeAnyActive() {
        boolean any = false;
        for (int i = 0; i < 6; i++) any |= this.indicators[i].active;
        this.anyActive = any;
    }
    ...
    public class IndicatorUnit {
        ...
        public void update() {
            ...
            RTTYChannel chan = RTTYSystem.listen(world, rtty);
            int sigVal = 0;

            if (chan != null && chan.timeStamp < world.getTotalWorldTime() - 1) chan = null;
    ...
    public GuiScreen provideGUI(int ID, EntityPlayer player, World world, int x, int y, int z) {
        return new GUIScreenRBMKIndicator(this);
    }
    ...
    public void receiveControl(NBTTagCompound data) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            indicator.color = MathHelper.clamp(data.getInteger("color" + i), 0, 0xffffff);
