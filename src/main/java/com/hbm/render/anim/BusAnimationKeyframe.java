...
public class BusAnimationKeyframe {
	...
	public final double legacyX;
	public final double legacyY;
	public final double legacyZ;
	public final boolean legacyTriplet;
	...
	public BusAnimationKeyframe() {
		...
		this.legacyX = this.legacyY = this.legacyZ = 0;
		this.legacyTriplet = false;
	}
	...
	public BusAnimationKeyframe(double x, double y, double z, int duration) {
		...
		this.legacyY = y;
		this.legacyZ = z;
		this.legacyTriplet = true;
	}

	public BusAnimationKeyframe(double x, double y, double z, int duration, InterpolationType interpolation) {
		this(x, y, z, duration);
		this.interpolationType = interpolation == InterpolationType.NONE ? IType.CONSTANT : IType.LINEAR;
	}

	public double[] toArray() {
		return new double[] { legacyX, legacyY, legacyZ };
	}
	...
	private double sqrt3(double d) {
		if (d > 0.000001) return Math.exp(Math.log(d) / 3.0);
		else if (d > -0.000001) return 0;
		else return -Math.exp(Math.log(-d) / 3.0);
	}
	...
	double BLI_easing_bounce_ease_out(double time, double begin, double change, double duration) {
		time /= duration;
		if (time < (1 / 2.75f)) return change * (7.5625f * time * time) + begin;
		if (time < (2 / 2.75f)) {
			time -= (1.5f / 2.75f);
	...
	double BLI_easing_bounce_ease_in_out(double time, double begin, double change, double duration) {
		if (time < duration / 2) return BLI_easing_bounce_ease_in(time * 2, 0, change, duration) * 0.5f + begin;
	...
	double BLI_easing_circ_ease_in_out(double time, double begin, double change, double duration) {
		if ((time /= duration / 2) < 1.0f) return -change / 2 * (Math.sqrt(1 - time * time) - 1) + begin;
		time -= 2.0f;
	...
	double BLI_easing_cubic_ease_in_out(double time, double begin, double change, double duration) {
		if ((time /= duration / 2) < 1.0f) return change / 2 * time * time * time + begin;
		time -= 2.0f;
	...
	double elastic_blend(double time, double change, double duration, double amplitude, double s, double f) {
		if (change != 0) {
			double t = Math.abs(s);
			if (amplitude != 0) f *= amplitude / Math.abs(change);
			else f = 0.0f;
	...
	double BLI_easing_elastic_ease_in(double time, double begin, double change, double duration, double amplitude, double period) {
		double s;
		double f = 1.0f;
		if (time == 0.0f) return begin;
		if ((time /= duration) == 1.0f) return begin + change;
		time -= 1.0f;
	...
	double BLI_easing_elastic_ease_in_out(double time, double begin, double change, double duration, double amplitude, double period) {
		...
		double f = 1.0f;
		if (time == 0.0f) return begin;
		if ((time /= duration / 2) == 2.0f) return begin + change;
		time -= 1.0f;
	...
	static final double pow_min = 0.0009765625f;
	...
	double BLI_easing_expo_ease_in_out(double time, double begin, double change, double duration) {
		...
		if (time <= duration_half) return BLI_easing_expo_ease_in(time, begin, change_half, duration_half);
	...
	double BLI_easing_quad_ease_in_out(double time, double begin, double change, double duration) {
		if ((time /= duration / 2) < 1.0f) return change / 2 * time * time + begin;
		time -= 1.0f;
	...
	double BLI_easing_quart_ease_in_out(double time, double begin, double change, double duration) {
		if ((time /= duration / 2) < 1.0f) return change / 2 * time * time * time * time + begin;
		time -= 2.0f;
	...
	double BLI_easing_quint_ease_in_out(double time, double begin, double change, double duration) {
		if ((time /= duration / 2) < 1.0f) return change / 2 * time * time * time * time * time + begin;
		time -= 2.0f;
