...
public class BusAnimationKeyframeSedna {
    ...
    public BusAnimationKeyframeSedna() {
        this.value = 0;
    ...
    public BusAnimationKeyframeSedna(double value, int duration) {
        this();
        this.value = value;
        this.originalDuration = this.duration = (int) (duration / Math.max(0.001D, ClientConfig.GUN_ANIMATION_SPEED.get()));
    }

    public BusAnimationKeyframeSedna(double value, int duration, IType interpolation) {
        this(value, duration);
        this.interpolationType = interpolation;
    }

    public BusAnimationKeyframeSedna(double value, int duration, IType interpolation, EType easing) {
        this(value, duration, interpolation);
        this.easingType = easing;
    }

    public double interpolate(double startTime, double currentTime, BusAnimationKeyframeSedna previous) {
        if(previous == null)
            previous = new BusAnimationKeyframeSedna();
