...
public class FT_Toxin extends FluidTrait {
	...
	public static abstract class ToxinEntry {
		...
		public boolean isProtected(EntityLivingBase entity) {
			...
			if(clazz != null && ArmorRegistry.hasAllProtection(entity, EntityEquipmentSlot.HEAD, clazz)) {
	...
	public static class ToxinDirectDamage extends ToxinEntry {
		...
		public void poison(EntityLivingBase entity, double intensity) {
			...
			if(delay == 0 || entity.world.getTotalWorldTime() % delay == 0) {
	...
	public static class ToxinEffects extends ToxinEntry {
		...
		public void poison(EntityLivingBase entity, double intensity) {
			...
			for(PotionEffect eff : effects) {
				entity.addPotionEffect(new PotionEffect(eff.getPotion(), (int) (eff.getDuration() * intensity), eff.getAmplifier()));
			}
		}
		...
		for(ToxinEntry entry : entries) {
			...
			if(entry instanceof ToxinEffects) {
				...
				for(PotionEffect effect : e.effects) {
					writer.beginArray();
					writer.value(effect.getPotion().getRegistryName().toString()).value(effect.getDuration()).value(effect.getAmplifier()).value(effect.getIsAmbient());
					writer.endArray();
				}
				writer.endArray();
				writer.setIndent("  ");
		...
		for(int i = 0; i < array.size(); i++) {
			...
			if(name.equals("effects")) {
				...
				for(int j = 0; j < effects.size(); j++) {
					...
					Potion potionN = Potion.REGISTRY.getObject(new ResourceLocation(effect.get(0).getAsString()));
					if (potionN != null) {
						PotionEffect potion = new PotionEffect(potionN,
