...
public class AnnihilatorRecipes extends SerializableRecipe {
        ...
        for(Entry<Object, AnnihilatorRecipe> entry : AnnihilatorRecipes.recipes.entrySet()) {
            for(Pair<BigInteger, ItemStack> milestone : entry.getValue().milestones) {
                ...
                if(input instanceof List) {
                    ...
                    for(ItemStack stack : list) ItemStackUtil.addTooltipToStack(stack, TextFormatting.RED + String.format(Locale.US, "%,d", milestone.getKey()));
    ...
    public static ItemStack getHighestPayoutFromKey(Object key, BigInteger prevAmount, BigInteger currentAmount) {
        ...
        if(recipe != null) {
            return getHighestPayoutFromRecipe(recipe, prevAmount, currentAmount);
        }
        return null;
    }
    ...
    public void readRecipe(JsonElement recipe) {
        ...
        if("item".equals(keyType)) {
            keyObject = Item.REGISTRY.getObject(new ResourceLocation(key.get("item").getAsString()));
        }
        if("comp".equals(keyType)) {
            keyObject = new ComparableStack(Item.REGISTRY.getObject(new ResourceLocation(key.get("item").getAsString())), 1, key.get("meta").getAsInt());
        }
        ...
        for(JsonElement e : milestones) {
            ...
            milestoneList.add(new Pair<>(milestone.get("amount").getAsBigInteger(), readItemStack(milestone.get("payout").getAsJsonArray())));
        }
    ...
    public void writeRecipe(Object recipe, JsonWriter writer) throws IOException {
        ...
        if(rec.getKey() instanceof Item item) {
            writer.name("type").value("item");
            writer.name("item").value(Item.REGISTRY.getNameForObject(item).toString());
        }
        if(rec.getKey() instanceof ComparableStack comp) {
            writer.name("type").value("comp");
            writer.name("item").value(Item.REGISTRY.getNameForObject(comp.item).toString());
            writer.name("meta").value(comp.meta);
        }
        if(rec.getKey() instanceof FluidType fluid) {
            writer.name("type").value("fluid");
            writer.name("fluid").value(fluid.getTranslationKey());
        }
        ...
        for(Pair<BigInteger, ItemStack> milestone : rec.getValue().milestones) {
            ...
            writer.name("payout");
            writeItemStack(milestone.getValue(), writer);
            writer.endObject();
        }
        writer.endArray();
    }
