...
public class AnvilRecipes extends SerializableRecipe {
	...
	public static void registerSmithing() {
		...
		smithingRecipes.add(new AnvilSmithingRecipe(4, new ItemStack(ModItems.flask_infusion, 1, ItemFlask.EnumInfusion.SHIELD.ordinal()), new ComparableStack(ModItems.gem_alexandrite), new ComparableStack(ModItems.bottle_nuka)));
		...
		smithingRecipes.add(new AnvilSmithingMold(15, new OreDictStack(IRON.plateCast(), 3), new OreDictStack("plateTriple", 3)));
	...
	public static void registerConstructionSirens() {
		for(ItemCassette.TrackType track : ItemCassette.TrackType.VALUES.values()) {
			if(track == ItemCassette.TrackType.NULL) continue;
			constructionRecipes.add(new AnvilConstructionRecipe(new AStack[] {new OreDictStack(STEEL.plate()), new ComparableStack(ModItems.plate_polymer)}, new AnvilOutput(new ItemStack(ModItems.siren_track, 1, track.getId()))).setTier(2).setOverlay(OverlayType.CONSTRUCTION));
		}
	}
	...
	public static void registerConstructionRecycling() {
		...
		constructionRecipes.add(new AnvilConstructionRecipe(
				new ComparableStack(DictFrame.fromOne(ModItems.chunk_ore, ItemEnums.EnumChunkType.RARE)),
				new AnvilOutput[] {
	...
	public static class AnvilConstructionRecipe {
		...
		public AnvilConstructionRecipe(AStack[] input, Tuple.Pair<ItemStack, Float>[] output) {
			Collections.addAll(this.input, input);
			for(Tuple.Pair<ItemStack, Float> out : output) this.output.add(new AnvilOutput(out.getKey(), out.getValue()));
			this.setOverlay(OverlayType.NONE); //no preferred overlay for many:many conversions
		}
