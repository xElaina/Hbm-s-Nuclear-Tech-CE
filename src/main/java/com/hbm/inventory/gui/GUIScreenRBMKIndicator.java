...
public class GUIScreenRBMKIndicator extends GuiScreen {
	...
	private static ResourceLocation texture = new ResourceLocation(Tags.MODID + ":textures/gui/machine/gui_rbmk_indicator.png");
	...
	public void initGui() {
		...
		for(int i = 0; i < 6; i++) {
			...
			color[i] = new GuiTextField(0, this.fontRenderer, guiLeft + 27 + oX, guiTop + 37 + oY + i * 36, 72 - oX * 2, 14);
			GUIScreenRBMKKeyPad.setupTextFieldStandard(color[i], 6, col);
			label[i] = new GuiTextField(0, this.fontRenderer, guiLeft + 175 + oX, guiTop + 37 + oY + i * 36, 72 - oX * 2, 14);
			...
			min[i] = new GuiTextField(0, this.fontRenderer, guiLeft + 121 + oX, guiTop + 55 + oY + i * 36, 52 - oX * 2, 14);
			GUIScreenRBMKKeyPad.setupTextFieldStandard(min[i], 32, indicator.indicators[i].min + "");
			max[i] = new GuiTextField(0, this.fontRenderer, guiLeft + 195 + oX, guiTop + 55 + oY + i * 36, 52 - oX * 2, 14);
	...
	protected void mouseClicked(int x, int y, int b) throws IOException {
		...
		if(guiLeft + 209 <= x && guiLeft + 209 + 18 > x && guiTop + 8 < y && guiTop + 8 + 18 >= y) {
			...
			PacketDispatcher.wrapper.sendToServer(new NBTControlPacket(data, indicator.getPos().getX(), indicator.getPos().getY(), indicator.getPos().getZ()));
			return;
		}
