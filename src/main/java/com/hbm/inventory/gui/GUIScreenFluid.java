...
public class GUIScreenFluid extends GuiScreen {
    ...
    public void updateScreen() {
        if(player.getHeldItemMainhand().isEmpty() || player.getHeldItemMainhand().getItem() != ModItems.fluid_identifier_multi)
            player.closeScreen();
    }
    ...
    public void initGui() {
        ...
        this.search = new GuiTextField(0, this.fontRenderer, guiLeft + 46, guiTop + 11, 86, 12);
        ...
        if(!player.getHeldItemMainhand().isEmpty() && player.getHeldItemMainhand().getItem() == ModItems.fluid_identifier_multi) {
            this.primary = ItemFluidIDMulti.getType(player.getHeldItemMainhand(), true);
            this.secondary = ItemFluidIDMulti.getType(player.getHeldItemMainhand(), false);
        }
    }
    ...
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        ...
        for(int k = 0; k < this.searchArray.length; k++) {
            ...
            if(guiLeft + 7 + k * 18 <= i && guiLeft + 7 + k * 18 + 18 > i && guiTop + 29 < j && guiTop + 29 + 18 >= j)
                drawHoveringText(Arrays.asList(new String[] { this.searchArray[k].getLocalizedName() }), i, j);
        }
    }
